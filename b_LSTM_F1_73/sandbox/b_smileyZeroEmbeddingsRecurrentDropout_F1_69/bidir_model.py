...
from keras.layers import Dense, Embedding,GRU, LSTM, Input, Concatenate, Dropout, Bidirectional, Reshape, Flatten
...
def buildModel(embeddingMatrix, smileyEmbeddings):
    ...
    lstm_person1 = Bidirectional(LSTM(LSTM_DIM, dropout=DROPOUT,recurrent_dropout=0.5, return_sequences=True))
    lstm_person2 = Bidirectional(LSTM(LSTM_DIM, dropout=DROPOUT,recurrent_dropout=0.5, return_sequences=True))
    lstm_smiley = Bidirectional(LSTM(int(LSTM_DIM/2), dropout=DROPOUT,recurrent_dropout=0.5, return_sequences=False))
