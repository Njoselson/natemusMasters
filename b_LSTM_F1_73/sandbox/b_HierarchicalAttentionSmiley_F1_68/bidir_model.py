...
def buildModel(embeddingMatrix, smileyEmbeddings):
    ...
    embeddingLayer = Embedding(embeddingMatrix.shape[0],EMBEDDING_DIM,
                                weights=[embeddingMatrix],input_length=MAX_SEQUENCE_LENGTH,
                                mask_zero=True,trainable=True)
    smileyEmbeddingLayer = Embedding(smileyEmbeddings.shape[0], 300,
                                     weights=[smileyEmbeddings], input_length=MAX_SMILEY_LENGTH,
                                     mask_zero=True, trainable=True)

    word_input = Input(shape=(100,), dtype='float32')
    word_sequences = embeddingLayer(word_input)
    word_smileys = smileyEmbeddingLayer(word_input)
    sequences = Concatenate(axis=-1)([word_sequences, word_smileys])
    word_lstm = Bidirectional(LSTM(LSTM_DIM, return_sequences=True, dropout=DROPOUT))(sequences)
    word_dense = TimeDistributed(Dense(200))(word_lstm)
    word_att = Dropout(DROPOUT)(AttentionWithContext()(word_dense))
    wordEncoder = Model(word_input, word_att)

    sent_input = Input(shape=(3, 100), dtype='float32')
    sent_encoder = TimeDistributed(wordEncoder)(sent_input)
    sent_lstm = Bidirectional(LSTM(LSTM_DIM, return_sequences=True, dropout=DROPOUT))(sent_encoder)
    sent_dense = TimeDistributed(Dense(200))(sent_lstm)
    sent_att = Dropout(0.5)(AttentionWithContext()(sent_dense))
    preds = Dense(4, activation='sigmoid')(sent_att)
    model = Model(sent_input, preds)
    model.compile(loss='categorical_crossentropy',optimizer='adam',metrics=['acc'])
    ...
    # lstm3 = lstm(emb3)
    #
    # lstm4 = lstm_smiley(smiley_emb1)
    # lstm5 = lstm_smiley(smiley_emb2)
    # lstm6 = lstm_smiley(smiley_emb3)
    #
    # #full network
...
def main():
    ...
    trainDat = np.stack([u1_data,u2_data,u3_data],axis=1)
    valDat = np.stack([u1_valData,u2_valData,u3_valData],axis=1)
    testDat = np.stack([u1_testData,u2_testData,u3_testData],axis=1)
    ...
    if EARLY_STOPPING=='True':
        ...
        history = model.fit([trainDat], labels, validation_data=([valDat], validationLabels), epochs=NUM_EPOCHS, batch_size=BATCH_SIZE, verbose=2, callbacks=[es, mc])
    else:
        model = buildModel(embeddingMatrix, smileyEmbeddings)
    ...
    predictions = model.predict([testDat], batch_size=BATCH_SIZE)
