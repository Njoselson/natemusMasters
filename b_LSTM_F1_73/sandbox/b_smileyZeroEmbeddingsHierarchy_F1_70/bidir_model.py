...
def buildModel(embeddingMatrix, smileyEmbeddings):
    ...
    lstm_person1 = Bidirectional(LSTM(LSTM_DIM, dropout=DROPOUT2, return_sequences=True))
    lstm_person2 = Bidirectional(LSTM(LSTM_DIM, dropout=DROPOUT,recurrent_dropout=0.5, return_sequences=True))
    lstm_smiley = Bidirectional(LSTM(LSTM_DIM, dropout=DROPOUT,recurrent_dropout=0.5, return_sequences=True))

    lstm_final= Bidirectional(LSTM(LSTM_DIM, dropout=DROPOUT,recurrent_dropout=0.5, return_sequences=False))
    ...
    #full network
    concatenated_lstm = Concatenate(axis=1)([lstm1, lstm2, lstm3,lstm4])
    #reshaped_lstm = Flatten()(concatenated_lstm)

    #concatenated_smiley = Concatenate(axis=-1)([concatenated_lstm, ])

    #cool hidden
    #hidden_layer = Dense(256, activation='relu')(concatenated_smiley)
    #dropout = Dropout(DROPOUT)(hidden_layer)
    final_lstm = lstm_final(concatenated_lstm)
