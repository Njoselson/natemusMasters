...
# def f1(ground, predictions):
#     #discretePredictions = to_categorical(predictions.argmax(axis=1))
#     discretePredictions = predictions.sign(tf.maximum(x, 0))
#     truePositives = np.sum(discretePredictions*ground, axis=0)
#     falsePositives = np.sum(np.clip(discretePredictions - ground, 0, 1), axis=0)
#     falseNegatives = np.sum(np.clip(ground-discretePredictions, 0, 1), axis=0)
#     macroPrecision = 0
...
def main():
    ...
    validationIndices, validationTexts, validationLabels, u1_val, u2_val, u3_val = preprocessData(validationDataPath, mode="train")
    # writeNormalisedData(testDataPath, testTexts)
    print("Processing test data...")
    testIndices, testTexts, testLabels, u1_test, u2_test, u3_test = preprocessData(testDataPath, mode="train")
    ...
    u1_valSequences, u2_valSequences, u3_valSequences = tokenizer.texts_to_sequences(u1_val), tokenizer.texts_to_sequences(u2_val), tokenizer.texts_to_sequences(u3_val)
    ...
    if EARLY_STOPPING=='True':
        es = EarlyStopping(monitor='f1', mode='max', verbose=1, patience=5)
        mc = ModelCheckpoint('EP%d_LR%de-5_LDim%d_BS%d.h5'%(NUM_EPOCHS, int(LEARNING_RATE*(10**5)), LSTM_DIM, BATCH_SIZE), monitor='f1', mode='max', verbose=1, save_best_only=True)
        # fit model
        model = buildModel(embeddingMatrix)
        history = model.fit([u1_data,u2_data,u3_data], labels, validation_data=([u1_testData, u2_testData, u3_testData], testLabels), epochs=NUM_EPOCHS, batch_size=BATCH_SIZE, verbose=0, callbacks=[es, mc])
    else:
        model = buildModel(embeddingMatrix)
