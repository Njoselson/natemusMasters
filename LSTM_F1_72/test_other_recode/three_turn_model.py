...
def buildModel(embeddingMatrix, smileyEmbeddings):
    ...
    lstm_smiley = LSTM(LSTM_DIM, dropout=0.1)
    ...
    #full network
    concatenated_out = Concatenate(axis=-1)([lstm1, lstm2, lstm3, lstm_smiley])
    model_output = Dense(2, activation='sigmoid')(concatenated_out)
...
def main():
    ...
    validationLabels = list(map(lambda x: 1 if x>=1 else 0, validationLabels))
    # writeNormalisedData(testDataPath, testTexts)
    print("Processing test data...")
    testIndices, testTexts, testLabels, u1_test, u2_test, u3_test, smil_test = preprocessData(testDataPath, mode="train")
    testLabels = list(map(lambda x: 1 if x>=1 else 0, testLabels))
