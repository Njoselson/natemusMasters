...
def buildModel(embeddingMatrix):
    ...
    # model_output = Dense(4, activation='sigmoid')(concatenated_lstm)
...
def main():
    ...
    if EARLY_STOPPING=='True':
        ...
        #history = model.fit([u1_data,u2_data,u3_data, smiley_trial], labels, validation_data=([u1_valData,u2_valData,u3_valData, smiley_test], validationLabels), epochs=NUM_EPOCHS, batch_size=BATCH_SIZE, verbose=2, callbacks=[es, mc])
        history = model.fit([u1_data,u2_data,u3_data], labels, validation_data=([u1_testData,u2_testData,u3_testData], testLabels), epochs=NUM_EPOCHS, batch_size=BATCH_SIZE, verbose=2, callbacks=[es, mc])
    else:
        #model = buildModel(embeddingMatrix, smileyEmbeddings)
        model = buildModel(embeddingMatrix)
